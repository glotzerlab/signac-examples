...
@Project.operation.with_directives(
    workflow.cpu_directives(walltime=TRAIN_WALLTIME, n_cpu=N_CORES)
)
...
@workflow.training_group
@Project.operation.with_directives(
    workflow.cpu_directives(walltime=EVAL_WALLTIME, n_cpu=N_CORES)
)
...
def evaluation(job):
    ...
    vae.plot_latent(job=job, data_loader=val_loader, device=device)
    return
